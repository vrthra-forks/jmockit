...
public final class CurrencyConversionHttpClientFake extends MockUp<AbstractHttpClient>
{
   ...
   private String formatResultContainingCurrencyConversion(List<String> params)
   {
   ...
   private String getParameter(List<String> params, String name)
   {
//      Optional<String[]> param = params.stream().map(p -> p.split("=")).filter(p -> name.equals(p[0])).findAny();
//      return param.isPresent() ? param.get()[1] : null;
      for (String param : params) {
         String[] nameAndValue = param.split("=");

         if (name.equals(nameAndValue[0])) {
